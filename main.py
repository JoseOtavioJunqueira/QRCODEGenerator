...
layout = [
    ...
    [sg.Text('Coluna que contém o email do comprador (ex.: B):', size=(50, 1), font=('Any', 12)), sg.Input(key='colunaemail')],
...
def main():
    while True:
        ...
        if eventos == 'OK':
            ...
            base_url = "https://www.SAECOMP.com/?code="

            for i, row in enumerate(values):
                if row and row[0] != 'Conferiu Banco?' and row[0] != '':
                    unique_code = str(uuid.uuid4())
                    data = base_url + unique_code
